...
- 无论是概念，还是函数接口用法，消除二义性、歧义。做某事，尽可能不要给用户多的选择，尽可能精简概念、接口、实现方式。
...
- 抽象、具象是相对概念。在抽象层尽量抽象，但也可有能够接收具象参数的接口，具象层重载的接口内就可以decode/parse具象层的参数。这个参数可以是json等字符串甚至脚本。这不是什么OO思想，GPU可编程也就是这么回事。

- 无状态化编程，初学者都被告诫不要使用全局变量，实质就是对无状态化编程SAY NO。而滥用类是一个让状态化横行的原罪，类本身当然也并无罪（其实也有罪，类的H必须公开，而私有变量又必须写在类里，这相当矛盾。而后来出现的接口类的方法似乎可以改善这一点，可是额外写接口类却很累。所以我可能常会很不有没的在类CPP里弄static全局变量，也不想放到h作为成员），罪在滥用，但这个滥用确实来源于类这种事物的潜在误导，滥用类成员变量的危害性质同全局变量的危害其实类似。如果一个语言没有类，只有结构体（实体类）、方法聚合（行为类），是不是就可以逼着新手没法滥用？GO等语言有接口但无方法聚合。

- 团队很多成员工作职责只是填空，接口、其他模块“冷冻住”。

- 函数更高级形态可以说成是模块，之所以需要成其为模块，是因为会被重用，对外界使用尽可能是无状态化的，在逻辑流中被使用是无需考虑过多严格的先后顺序的才成其为模块，否则没有必要，不如平铺写在一大段代码里，没有必要分成一个一个函数。Craft的作者就有这么点这个意思。
...
- 线程安全的类或者函数，特意加上ts~前缀，如果另外还有c2~，则c2tx~，c2as~表示函数内部异步操作。
- x86/amd64/arm等移植话题。。。。部分请见event。
- _c2前缀东西意味着用户有机会看到，但是不会使用到。
- ThirdParty目录不一定是所有的外部库，而只是放有上层用户能用到的，我们并不进行封装的库。
- imgui公开含有的成熟库，犯不着我们脱了裤子放屁封装一层。
- 不使用自动指针，除了需要reference的地方（例如资源管理），就算是reference我们也考虑是不是显示调用。自动指针这种看似很好的机制其实是个搅屎棍，一个项目里有的地方坚持用，有的地方有不能用，会让编程范式混乱，而且还常常会让问题隐藏的更深。对于一个C程序员来讲，养成良好的释放资源的希望是必备的。哪怕是非C程序员，写程序经常要碰到释放某种资源的善后操作，不仅仅只是内存。
- 不要对第三方库做无意义的封装，但是我们还是要注意暴露给用户的接口不要有或者起码不要有过多的第三方依赖，除了一些明确肯定是让外界使用的，例如imgui的接口等（被直接copy到工程内的3rd，帮用户处理依赖项本来也会相对方便）。
- 若一个函数的参数里有OUT，那么IN的参数哪怕不是引用和指针（虽然保护的不是其自身），也用上CONST。
...
- Asset、Prefab、Templet等概念在Runtime里并不存在，Unity3D里好像也是如此。在应用开发工序流中存在的一种术语。从一定角度说，Asset是一种数据层视角的东西。
...
- Extension的一个基础话题，就是需要添加自己的事件类型chunk到Application基地事件系统内。
